...
index.controller("payTailorCtrl",["$scope","$http","$window","$location","$rootScope",function(e,r,t,o,a){e.customizeOrder=a.customizeOrder;var n=JSON.parse(sessionStorage.getItem("user"));e.user=n,e.toPay=function(){var t={type:"wz",orderid:e.customizeOrder.id};r.post("/pay/prepay.json",t,postCfg).success(function(e){if(1===e.code){var r=e.data;WeixinJSBridge&&WeixinJSBridge.invoke("getBrandWCPayRequest",{appId:r.appId,timeStamp:r.timeStamp.toString(),nonceStr:r.nonceStr,"package":r["package"],signType:r.signType,paySign:r.paySign},function(e){"get_brand_wcpay_request:ok"==e.err_msg?o.path("paySuccess"):alert("支付失败，请稍后再试！")})}else 0===e.code&&alert(e.reason)}).error(function(e){alert(e.reason)})}}]);
index.controller("personalCenterCtrl",["$scope","$http","$window","$location","$rootScope",function(o,n,t,r,e){var i=JSON.parse(sessionStorage.getItem("user"));o.user=i,o.editInformation=function(){window.location.href="/yueAssistant/build/html/editInformation"},o.personalTailor=function(){r.path("personalTailor")},o.goVipCenter=function(){r.path("vipCenter")},o.goWorks=function(){window.location.href="/yueAssistant/build/html/works"},o.goWorkHours=function(){r.path("workHours")}}]);
...
index.controller("useMornCtrl",["$scope","$http","$window","$location","$rootScope","$routeParams",function(t,e,l,o,a,r){function f(){t.btnText="立即使用",t.showcanvas=!0;var e=new Date,l=e.getFullYear()+"/"+(e.getMonth()+1)+"/"+e.getDate(),o=e.getMonth()+1;switch(o){case 1:o="J A N";break;case 2:o="F E B";break;case 3:o="M A R";break;case 4:o="A P R";break;case 5:o="M A Y";break;case 6:o="J U N";break;case 7:o="J U L";break;case 8:o="A U G";break;case 9:o="S E P";break;case 10:o="O C T";break;case 11:o="N O V";break;case 12:o="D E C"}var a=e.getDay();switch(a){case 0:a="S U N D A Y";break;case 1:a="M O N D A Y";break;case 2:a="T U E S D A Y";break;case 3:a="W E D N E S D A Y";break;case 4:a="T H U R S D A Y";break;case 5:a="F R I D A Y";break;case 6:a="S A T U R D A Y"}var r=n("main");r.width,r.height;r.clearRect(0,0,1e3,1e3);var f=new Image;f.src=t.showImg,f.onload=function(){if(r.drawImage(f,0,0,h,g),navigator.userAgent.match(/iphone/i)){if(1==c){r.font="normal bold 36px myFirstFont",r.fillStyle="black";var n=Math.floor(.544*m),i=Math.floor(.95*g);r.fillText("BY"+t.user.name,n,i)}else if(2==c){r.font="normal bold 30px myFirstFont",r.fillStyle="black";var s=Math.floor(.07*m),M=Math.floor(.906*g),x=Math.floor(.943*g);r.fillText("BY"+t.user.name,s,M),r.fillText(l,s,x)}else if(3==c){r.font="normal bold 100px myFirstFont",r.fillStyle="white";var b=Math.floor(.103*m),d=Math.floor(.87*g),y=Math.floor(.91*g),T=Math.floor(.947*g);r.fillText(e.getDate(),b,d),r.font="normal bold 30px myFirstFont",r.fillText(o+"."+a,b,y),r.fillText("BY"+t.user.name,b,T)}else if(4==c){r.font="normal bold 34px myFirstFont",r.fillStyle="black",r.textAlign="right";var F=Math.floor(.924*m),u=Math.floor(.145*g),p=Math.floor(.184*g),v=Math.floor(.223*g);r.fillText(l,F,u),r.fillText(a,F,p),r.fillText("BY"+t.user.name,F,v)}else if(5==c){r.font="normal bold 34px myFirstFont",r.fillStyle="#7a8776";var k=Math.floor(.773*m),S=Math.floor(.736*g);r.fillText("BY"+t.user.name,k,S)}else if(6==c){r.font="normal bold 0.46rem myFirstFont",r.fillStyle="#fff";var Y=Math.floor(.047*m),B=Math.floor(.915*g);r.fillText("BY"+t.user.name,Y,B)}else if(7==c){r.font="normal bold 30px myFirstFont",r.fillStyle="#000";var w=Math.floor(.07*m),D=Math.floor(.856*g),A=Math.floor(.883*g),I=Math.floor(.91*g);r.fillText(l,w,D),r.fillText(a,w,A),r.fillText("BY"+t.user.name,w,I)}else if(8==c){r.font="normal bold 100px myFirstFont",r.fillStyle="#96bfad";var $=Math.floor(.07*m),E=Math.floor(.106*g),U=Math.floor(.146*g),N=Math.floor(.186*g);r.fillText(e.getDate(),$,E),r.font="normal bold 30px myFirstFont",r.fillText(a,$,U),r.fillText("BY"+t.user.name,$,N)}}else if(1==c){r.font="normal bold 0.48rem myFirstFont",r.fillStyle="black";var R=Math.floor(.544*m),C=Math.floor(.95*g);r.fillText("BY"+t.user.name,R,C)}else if(2==c){r.font="normal bold 0.4rem myFirstFont",r.fillStyle="black";var J=Math.floor(.07*m),O=Math.floor(.906*g),P=Math.floor(.943*g);r.fillText("BY"+t.user.name,J,O),r.fillText(l,J,P)}else if(3==c){r.font="normal bold 1.333333rem myFirstFont",r.fillStyle="white";var _=Math.floor(.103*m),L=Math.floor(.87*g),W=Math.floor(.91*g),j=Math.floor(.947*g);r.fillText(e.getDate(),_,L),r.font="normal bold 0.4rem myFirstFont",r.fillText(o+"."+a,_,W),r.fillText("BY"+t.user.name,_,j)}else if(4==c){r.font="normal bold 0.453333rem myFirstFont",r.fillStyle="black",r.textAlign="right";var G=Math.floor(.924*m),H=Math.floor(.145*g),V=Math.floor(.184*g),q=Math.floor(.223*g);r.fillText(l,G,H),r.fillText(a,G,V),r.fillText("BY"+t.user.name,G,q)}else if(5==c){r.font="normal bold 0.453333rem myFirstFont",r.fillStyle="#7a8776";var z=Math.floor(.773*m),K=Math.floor(.736*g);r.fillText("BY"+t.user.name,z,K)}else if(6==c){r.font="normal bold 0.46rem myFirstFont",r.fillStyle="#fff";var Q=Math.floor(.047*m),X=Math.floor(.915*g);r.fillText("BY"+t.user.name,Q,X)}else if(7==c){r.font="normal bold 0.4rem myFirstFont",r.fillStyle="#000";var Z=Math.floor(.07*m),tt=Math.floor(.856*g),et=Math.floor(.883*g),lt=Math.floor(.91*g);r.fillText(l,Z,tt),r.fillText(a,Z,et),r.fillText("BY"+t.user.name,Z,lt)}else if(8==c){r.font="normal bold 1.333333rem myFirstFont",r.fillStyle="#96bfad";var ot=Math.floor(.07*m),at=Math.floor(.106*g),rt=Math.floor(.146*g),ft=Math.floor(.186*g);r.fillText(e.getDate(),ot,at),r.font="normal bold 0.4rem myFirstFont",r.fillText(a,ot,rt),r.fillText("BY"+t.user.name,ot,ft)}}}function n(t){return document.getElementById(t).getContext("2d")}function i(){t.showcanvas=!1;var e=document.getElementById("main"),l=e.toDataURL("image/jpg");t.canvasImg=l,t.btnText="长按保存至手机，再发到朋友圈炫耀"}var s=JSON.parse(sessionStorage.getItem("user"));t.user=s,t.btnText="立即使用",t.loading=!1,t.showcanvas=!0,t.canvasImg="";var m=document.documentElement.clientWidth,h=Math.floor(m),g=Math.floor(1.83*m);$("#main").attr("width",h+"px"),$("#main").attr("height",g+"px"),console.log(t.user);var c=r.num;t.page=1,t.showImg="../../assets/images/morning/type"+c+"/img_"+t.page+".png";for(var M=1;6>M;M++){var x="../../assets/images/goodnight/type"+c+"/img_"+M+".png";$(".hideImg").append("<img src="+x+">")}t.editInformation=function(){o.path("editInformation")},t.useTemplatePage=function(){i(),t.btnText="长按保存至手机，再发到朋友圈炫耀"},t.goBack=function(){l.history.back()},t.closeText=function(){t.user.iscomplete=!0},t.changeImg=function(){t.loading=!0,t.page>=5?(t.page=1,t.showImg="../../assets/images/morning/type"+c+"/img_"+t.page+".png",t.loading=!1):(t.page+=1,t.showImg="../../assets/images/morning/type"+c+"/img_"+t.page+".png",t.loading=!1),f()},$(function(){f()})}]);
